...
async fn handler(
    ...
    // Step 3: Invoke an external Lambda for fulfillment (wrapped in step).
    let fulfillment_fn =
        std::env::var("FULFILLMENT_FUNCTION").unwrap_or_else(|_| "fulfillment-lambda".to_string());
    ...
    // Step 5: Run post-processing (using step instead of child_context since
    // the durable execution service does not yet support Context operations).
