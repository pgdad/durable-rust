...
//! providing builder-approach ergonomics. All operations delegate directly
...
///
/// Thin wrapper over [`DurableContext`] providing the builder-approach API.
/// All operations delegate directly to the inner context — no replay logic,
/// no checkpoint logic, just delegation.
///
/// Constructed internally by [`DurableHandlerBuilder::run`](crate::DurableHandlerBuilder::run)
/// — users never create this directly.
///
/// # Examples
///
/// ```no_run
...
impl BuilderContext {
    ...
    ///
    /// This constructor is `pub(crate)` — only the
    /// [`DurableHandlerBuilder::run`](crate::DurableHandlerBuilder::run)
    /// method creates `BuilderContext` instances.
    ...
    ///
    /// ```no_run
    /// # async fn example(ctx: &mut durable_lambda_builder::context::BuilderContext) -> Result<(), durable_lambda_core::error::DurableError> {
    ...
    ///
    /// ```no_run
    /// # async fn example(ctx: &durable_lambda_builder::context::BuilderContext) {
...
mod tests {
    ...
    /// Mock backend for testing BuilderContext delegation.
    struct MockBackend {
    ...
    async fn make_builder_context(backend: MockBackend) -> BuilderContext {
    ...
    #[tokio::test]
    async fn test_builder_context_step_delegates_to_core() {
        let mut ctx = make_builder_context(MockBackend::new()).await;
    ...
    #[tokio::test]
    async fn test_builder_context_step_with_options_delegates_to_core() {
        let mut ctx = make_builder_context(MockBackend::new()).await;
    ...
    #[tokio::test]
    async fn test_builder_context_execution_mode_executing() {
        let ctx = make_builder_context(MockBackend::new()).await;
        assert_eq!(ctx.execution_mode(), ExecutionMode::Executing);
        assert!(!ctx.is_replaying());
    }

    #[tokio::test]
    async fn test_builder_context_execution_mode_replaying() {
        ...
        let ctx = BuilderContext::new(durable_ctx);
        assert_eq!(ctx.execution_mode(), ExecutionMode::Replaying);
        assert!(ctx.is_replaying());
    }

    #[tokio::test]
    async fn test_builder_context_arn() {
        let ctx = make_builder_context(MockBackend::new()).await;
        assert_eq!(ctx.arn(), "arn:test");
    }

    #[tokio::test]
    async fn test_builder_context_checkpoint_token() {
        let ctx = make_builder_context(MockBackend::new()).await;
        assert_eq!(ctx.checkpoint_token(), "tok");
    }

    #[tokio::test]
    async fn test_builder_context_child_context_delegates_to_core() {
        let mut ctx = make_builder_context(MockBackend::new()).await;
    ...
    #[tokio::test]
    async fn test_builder_context_log_delegates_to_core() {
        let ctx = make_builder_context(MockBackend::new()).await;
