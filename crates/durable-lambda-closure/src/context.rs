...
//! providing closure-approach ergonomics. All operations delegate directly
...
impl ClosureContext {
    ...
    /// function creates `ClosureContext` instances.
    ...
    ///
    /// ```no_run
    /// # async fn example(ctx: &durable_lambda_closure::context::ClosureContext) {
...
impl DurableContextOps for ClosureContext {
    ...
    fn parallel<T, F, Fut>(
        ...
        name: &str,
        branches: Vec<F>,
        options: durable_lambda_core::types::ParallelOptions,
...
mod tests {
    ...
    /// Mock backend for testing ClosureContext delegation.
    struct MockBackend {
    ...
    async fn make_closure_context(backend: MockBackend) -> ClosureContext {
    ...
    #[tokio::test]
    async fn test_closure_context_step_delegates_to_core() {
        let mut ctx = make_closure_context(MockBackend::new()).await;

        // In executing mode, step should execute the closure and checkpoint
    ...
    #[tokio::test]
    async fn test_closure_context_step_with_options_delegates_to_core() {
        let mut ctx = make_closure_context(MockBackend::new()).await;
    ...
    #[tokio::test]
    async fn test_closure_context_execution_mode_executing() {
        let ctx = make_closure_context(MockBackend::new()).await;
        assert_eq!(ctx.execution_mode(), ExecutionMode::Executing);
        assert!(!ctx.is_replaying());
    }

    #[tokio::test]
    async fn test_closure_context_execution_mode_replaying() {
        // Create a context with a completed operation to trigger replay mode.
        ...
        let ctx = ClosureContext::new(durable_ctx);
        assert_eq!(ctx.execution_mode(), ExecutionMode::Replaying);
        assert!(ctx.is_replaying());
    }

    #[tokio::test]
    async fn test_closure_context_arn() {
        let ctx = make_closure_context(MockBackend::new()).await;
        assert_eq!(ctx.arn(), "arn:test");
    }

    #[tokio::test]
    async fn test_closure_context_checkpoint_token() {
        let ctx = make_closure_context(MockBackend::new()).await;
        assert_eq!(ctx.checkpoint_token(), "tok");
    }

    #[tokio::test]
    async fn test_closure_context_child_context_delegates_to_core() {
        let mut ctx = make_closure_context(MockBackend::new()).await;
    ...
    #[tokio::test]
    async fn test_closure_context_log_delegates_to_core() {
        let ctx = make_closure_context(MockBackend::new()).await;
        // Verify log methods are callable and don't panic.
        // In executing mode, these delegate to DurableContext which emits tracing events.
